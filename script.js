...
const MAX_RADIUS = 2000;   // metres — default search radius
const MAX_RESULTS = 50;    // cap on displayed results

// ── STATE ─────────────────────────────────────────
let userLat = null;
let userLng = null;
let allResults = [];        // raw API results
let filteredResults = [];   // after filter pills + borough text
...
// ── API FETCH ──────────────────────────────────────
/**
 * Socrata SODA API — no backend needed.
 * We use $where with within_circle() to filter by lat/lng radius server-side.
 * Results come back as JSON directly in the browser.
 * The API is CORS-enabled and publicly accessible without auth
 * (though throttled without an app token — register one free at
...
function renderResults() {
  if (filteredResults.length === 0 && allResults.length > 0) {
    ...
    showResultsList();   // show empty list
...
// ── UTILS ─────────────────────────────────────────
/**
 * Haversine distance formula — returns metres between two lat/lng pairs.
 */
